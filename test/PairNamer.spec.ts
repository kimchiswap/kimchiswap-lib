...
describe('PairNamer', () => {
  ...
  describe('#pairName', () => {
    ...
    it('concatenation', async () => {
      expect(await pairName('DAI Stable Coin', 'MKR Token', 'Kswap V1 Liquidity Provider Share for ', '.')).to.eq(
        'Kswap V1 Liquidity Provider Share for DAI Stable Coin:MKR Token.'
      )
    })
  })
  ...
  describe('#pairSymbol', () => {
    ...
    it('concatenation', async () => {
      expect(await pairSymbol('DAI', 'MKR', '-v2')).to.eq('🌶️DAI:MKR-v2')
    })
  })
})
